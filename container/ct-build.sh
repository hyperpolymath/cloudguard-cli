...
#
# cloudguard-cli — Cerro Torre build, sign, and verify pipeline
#
...
#   CT_KEY_ID       — Signing key identifier (default: cloudguard-cli-release)
#   CT_REGISTRY     — OCI registry to push to (default: cgr.dev/chainguard)
...
CT_KEY_ID="${CT_KEY_ID:-cloudguard-cli-release}"
CT_REGISTRY="${CT_REGISTRY:-cgr.dev/chainguard}"
CT_TAG="${CT_TAG:-latest}"

IMAGE_NAME="cloudguard-cli"
FULL_IMAGE="${CT_REGISTRY}/${IMAGE_NAME}:${CT_TAG}"
CTP_FILE="${SCRIPT_DIR}/${IMAGE_NAME}-${CT_TAG}.ctp"

echo "=== cloudguard-cli Cerro Torre Build Pipeline ==="
