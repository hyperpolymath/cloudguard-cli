...
init:
    ...
    PATTERN="${LB}[A-Z_]*${RB}"
    REMAINING=$(grep -rl "$PATTERN" . --include='*.md' --include='*.adoc' --include='*.yml' --include='*.yaml' --include='*.a2ml' --include='*.toml' --include='*.scm' --include='*.ncl' --include='*.guix' --include='*.json' --include='*.sh' 2>/dev/null | grep -v '.git/' | grep -v 'PLACEHOLDERS.md' || true)
    if [ -n "$REMAINING" ]; then
...
build *args:
    @echo "Building cloudguard_cli (debug)..."
...
build-release *args:
    @echo "Building cloudguard_cli (release)..."
...
install: build-release
    @echo "Installing cloudguard_cli..."
...
cookbook:
    ...
    echo "= cloudguard_cli Justfile Cookbook" > "$OUTPUT"
...
man:
    #!/usr/bin/env bash
    mkdir -p docs/man
    cat > docs/man/cloudguard_cli.1 << EOF
    .TH cloudguard_cli 1 "$(date +%Y-%m-%d)" "{{version}}" "cloudguard_cli Manual"
    .SH NAME
    cloudguard_cli \- RSR-compliant project
    .SH SYNOPSIS
    .B just
    [recipe] [args...]
    .SH DESCRIPTION
    ...
    EOF
    echo "Generated: docs/man/cloudguard_cli.1"
...
container-init:
    ...
    read -rp "Service name (e.g. my-api) [cloudguard_cli]: " _SERVICE_NAME
    SERVICE_NAME="${_SERVICE_NAME:-cloudguard_cli}"
...
container-build *args:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman build -t cloudguard_cli:latest -f container/Containerfile .
    elif [ -f "Containerfile" ]; then
        podman build -t cloudguard_cli:latest -f Containerfile .
    else
...
container-push:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman push cloudguard_cli:latest
    fi

# Run container interactively (for debugging)
container-run *args:
    podman run --rm -it cloudguard_cli:latest {{args}}
