...
# CLOUDGUARD_CLI ABI/FFI Documentation
...
│  generated/abi/cloudguard_cli.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libcloudguard_cli.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── cloudguard_cli.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── cloudguard_cli.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/cloudguard_cli.h
```
...
```c
#include "cloudguard_cli.h"

int main() {
    void* handle = cloudguard_cli_init();
    if (!handle) return 1;

    int result = cloudguard_cli_process(handle, 42);
    if (result != 0) {
        const char* err = cloudguard_cli_last_error();
...
Compile with:
```bash
gcc -o example example.c -lcloudguard_cli -L./zig-out/lib
```

### From Idris2

```idris
import CLOUDGUARD_CLI.ABI.Foreign
...
```rust
#[link(name = "cloudguard_cli")]
extern "C" {
    fn cloudguard_cli_init() -> *mut std::ffi::c_void;
    fn cloudguard_cli_free(handle: *mut std::ffi::c_void);
    fn cloudguard_cli_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = cloudguard_cli_init();
        assert!(!handle.is_null());

        let result = cloudguard_cli_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libcloudguard_cli = "libcloudguard_cli"

function init()
    handle = ccall((:cloudguard_cli_init, libcloudguard_cli), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:cloudguard_cli_process, libcloudguard_cli), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:cloudguard_cli_free, libcloudguard_cli), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/cloudguard_cli.h
   ```
